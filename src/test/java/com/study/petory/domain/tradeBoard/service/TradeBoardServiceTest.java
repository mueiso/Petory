...
class TradeBoardServiceTest {
	...
	void 거래_게시글_생성에_성공한다() {
		...
		//when
		TradeBoardCreateResponseDto responseDto = tradeBoardService.saveTradeBoard(requestDto, images);
	...
	void 유저별_게시글_조회에_성공한다() {
		...
		when(tradeBoardRepository.findByUser(user, pageable)).thenReturn(page);
	...
	void 게시글_수정에_성공한다() {
		...
		//when
		TradeBoardUpdateResponseDto responseDto = tradeBoardService.updateTradeBoard(1L, requestDto);
