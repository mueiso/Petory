...
class ChatServiceImplTest {
	...
	void setUp() {
		...
		UserPrivateInfo userInfo1 = new UserPrivateInfo(1L, "name1", "01012341234");
		this.user1 = new User("nickname1", "test1@test.com", userInfo1, userRoles);
		ReflectionTestUtils.setField(user1, "id", 1L);

		UserPrivateInfo userInfo2 = new UserPrivateInfo(2L, "name2", "01098765432");
	...
	void 채팅방_생성에_성공한다() {
		...
		// save가 호출될 때, id를 직접 세팅해서 반환
		when(chatRepository.save(any(ChatRoom.class)))
			.thenAnswer(invocation -> {
				ChatRoom chatRoom = invocation.getArgument(0); //save 호출 시 반환되는 값
				ReflectionTestUtils.setField(chatRoom, "id", new ObjectId());
				return chatRoom;
			});

		// when
		ChatRoomCreateResponseDto responseDto = chatService.saveChatRoom(tradeBoardId);
		...
		assertThat(responseDto.getCustomerId()).isEqualTo(2L);
	...
	void 채팅방_단건_조회_성공() {
		...
		// when
		ChatRoomGetResponseDto responseDto = chatService.findChatRoomById(chatRoomId);
